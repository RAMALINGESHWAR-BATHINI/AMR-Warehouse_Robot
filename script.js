...
function sendAutoParkPath() {
    const parkX = 0;
    const parkY = 0;
    const path = findPath(robotPosition.x, robotPosition.y, parkX, parkY);
    if (path.length === 0) return;
    drawPath(path);
    latestPath = path;
...
setInterval(() => {
    const now = Date.now();
    if (now - lastCommandTime >= 2 * 60 * 1000) {
        sendAutoParkPath();
        updateStatus("Idle 2 min — Returning to parking.");
    }
}, 60000);
